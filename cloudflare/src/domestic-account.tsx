...
export function DomesticLogin() {
  ...
  return <main className="domestic-login-shell">
    <section className="domestic-login-card">
      ...
      <small>站长使用部署时配置的主管理员邮箱和密码。密码只在服务端校验，不会写入网页。</small>
    </section>
  </main>;
}
...
type Overview = {
  ...
  cards: { users: number; activeMembers: number; rollingGenerations: number };
  systems: { deepSeekConfigured: boolean; model: string; database: boolean; rollingWindowHours: number; freeLimit: number; memberLimit: number };
  users: Array<{ email: string; display_name: string | null; plan: string; plan_expires_at: number | null; last_seen_at: number }>;
  orders: Array<{ id: string; user_email: string; product: string; amount_fen: number; status: string; created_at: number }>;
...
export function DomesticAdmin() {
  ...
  if (!data) return <main className="domestic-admin-shell"><p className={error ? "domestic-login-error" : ""}>{error || "正在读取真实运营数据…"}</p></main>;
  return <main className="domestic-admin-shell">
    <header>
    ...
    <section className="domestic-admin-panel">
      ...
      <div className="domestic-order-list">{data.orders.length ? data.orders.map(order => {
        ...
        const status = order.status === "paid" ? "已核对到账" : order.status === "complimentary" ? "已赠送" : order.status;
        return <div key={order.id}><span><b>{order.user_email}</b><small>{order.product} · {method}</small></span><strong>¥{(order.amount_fen / 100).toFixed(2)}</strong><em className={order.status === "paid" ? "ok" : ""}>{status}</em><time>{new Date(order.created_at).toLocaleString("zh-CN")}</time></div>;
